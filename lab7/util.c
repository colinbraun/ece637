...
int clip_grayscale_image(double **img, size_t width, size_t height) {
    int clip_required = 0;
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
            if (img[row][col] > 255) {
...
int clip_grayscale_image_i(unsigned int **img, size_t width, size_t height) {
    int clip_required = 0;
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
            if (img[row][col] > 255) {
...
{
    // Clip the image
    clip_grayscale_image_i(img, width, height);
    struct TIFF_img image;
    get_TIFF ( &image, height, width, 'g' );
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
